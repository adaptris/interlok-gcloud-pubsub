...
@DisplayOrder(order = { "projectName", "credentialsProvider", "flowControlProvider", "channelProvider" })
public class GoogleCloudPubSubConnection extends ConnectionConfig {
  ...
  public ProjectSubscriptionName createSubscription(ConsumeConfig config) throws CoreException {
    SubscriptionName subscriptionName = SubscriptionName.of(getProjectName(), config.getSubscriptionName());
    ...
    try {
      ...
      log.trace("Found existing subscription [{}] for Topic [{}]", config.getSubscriptionName(), subscription.getTopic());
      if (!subscription.getTopic().equals(topic.toString())) {
        throw new CoreException(
            String.format("Existing subscription topics do not match [%s] [%s]", subscription.getTopic(), topic.toString()));
      }
      return ProjectSubscriptionName.parse(subscription.getName());
    } catch (ApiException e) {
      if (StatusCode.Code.NOT_FOUND != e.getStatusCode().getCode() || !config.getCreateSubscription()) {
        throw new CoreException("Failed to retrieve Topic", e);
      } else {
        log.trace("Creating Subscription [{}] Topic [{}]", config.getSubscriptionName(), topic.toString());
        // could cast to TopicName since ProjectTopicName extends TopicName to avoid the
        // deprecation warning
        Subscription subscription = getSubscriptionAdminClient().createSubscription(subscriptionName, topic,
  ...
  public void deleteSubscription(ConsumeConfig config) throws CoreException {
    SubscriptionName subscription = SubscriptionName.of(getProjectName(), config.getSubscriptionName());
    if (config.getCreateSubscription()) {
      log.trace("Deleting Subscription [{}] for Project [{}] Topic [{}]", config.getSubscriptionName(), getProjectName(),
          config.getTopicName());
  ...
  public Subscriber createSubscriber(ProjectSubscriptionName subscription, MessageReceiver receiver) {
    Subscriber.Builder subscriberBuilder = Subscriber.newBuilder(subscription, receiver).setChannelProvider(getGoogleChannelProvider())
